...
if __name__ == '__main__':
    ...
    ds = pypadre.pod.importing.dataset.graph_import.create_from_snap(snap_csv, 0)
    print("upload snap_csv graph to server")
    did = pypadre.pod.importing.dataset.ds_import.createServerDataset(ds, auth_token, server_url)
    print("download snap_csv graph to server")
    down = pypadre.pod.importing.dataset.ds_import.requestServerDataset(did, auth_token, server_url)
    down=None
    ds = None
    did=None
    ...
    ds = pypadre.pod.importing.dataset.graph_import.create_from_snap(snap_txt, 0)
    ...
    ds = pypadre.pod.importing.dataset.graph_import.create_from_snap(snap_biodata_tsv, 0)
    ...
    ds = pypadre.pod.importing.dataset.graph_import.create_from_konect(konect)
    ...
    ds = pypadre.pod.importing.dataset.ds_import.load_openML_dataset(openml_small, None, apikey="1f8766e1615225a727bdea12ad4c72fa")
    print("upload openml dataset to server")
    did = pypadre.pod.importing.dataset.ds_import.createServerDataset(ds, auth_token, server_url)
    ...
    ds = pypadre.pod.importing.dataset.ds_import.load_openML_dataset(openml_graph, None, apikey="1f8766e1615225a727bdea12ad4c72fa")
    print("upload openml dataset to server")

    print("convert to Graph 1")
    ds_g1= pypadre.pod.importing.dataset.graph_import.create_from_padre_dataset(ds, ds.attributes[0].name, ds.attributes[1].name)
    print("convert to Graph 2")
    ds_g2= pypadre.pod.importing.dataset.graph_import.create_from_padre_dataset(ds, ds.attributes[0].name, ds.attributes[1].name,
                                                                                node_attr=[], edge_attr=[ds.attributes[3].name,ds.attributes[4].name], directed=False)
    print("upload Graph 1 to server")
    did1 = pypadre.pod.importing.dataset.ds_import.createServerDataset(ds_g1, auth_token, server_url)
    print("upload Graph 2 to server")
    did2 = pypadre.pod.importing.dataset.ds_import.createServerDataset(ds_g2, auth_token, server_url)

    print("download Graph 1 from server")
    down1 = pypadre.pod.importing.dataset.ds_import.requestServerDataset(did1, auth_token, server_url)
    print("download Graph 2 from server")
    down2 = pypadre.pod.importing.dataset.ds_import.requestServerDataset(did2, auth_token, server_url)

    ds_from_df= pypadre.pod.importing.dataset.ds_import.load_pandas_df(ds.data, target_features=[ds.targets()])
    pypadre.pod.importing.dataset.ds_import.createServerDataset(ds_from_df, auth_token, url=server_url)
    #search on server for dataset did from name
    t_id= pypadre.pod.importing.dataset.ds_import.get_did_from_meta({"name":ds.name}, auth_token=auth_token, max_hits=2, url=server_url)

    print("upload standard sklearn_toys")
    for toy_ds in pypadre.pod.importing.dataset.ds_import.load_sklearn_toys():
        pypadre.pod.importing.dataset.ds_import.createServerDataset(toy_ds, auth_token, url=server_url)

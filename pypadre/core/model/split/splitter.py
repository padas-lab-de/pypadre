...
class Splitter:
    ...
    def splits(self, *, data, execution, component, **kwargs):
        """
        ...
        def splitting_iterator():
            ...
            if self._strategy is None:
                ...
                if self._val_ratio > 0:  # create a validation set out of the test set
                    ...
                    yield Split(execution=execution, num=++num, train_idx=train[:n_v], test_idx=test, val_idx=train[n_v:], component=component)
                else:
                    yield Split(execution=execution, num=++num, train_idx=train, test_idx=test, val_idx=None, component=component)
                ...
                for i in range(self._n_folds):
                    ...
                    if self._val_ratio > 0:  # create a validation set out of the test set
                        n_v = int(len(train) * self._val_ratio)
                        yield Split(execution, ++num, train[:n_v], test, train[n_v:], component=component)
                    else:
                        yield Split(execution, ++num, train, test, None, component=component)
                ...
                for i in range(len(self._index_list)):
                    ...
                    yield Split(execution, ++num, train, test, val, component=component)

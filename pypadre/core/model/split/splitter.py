...
class Splitter:
    ...
    def splits(self, *, data, run, component, predecessor=None, **kwargs):
        """
        ...
        def splitting_iterator():
            ...
            if self._strategy is None:
                yield Split(run=run, num=++num, train_idx=idx, test_idx=None, val_idx=None, component=component, predecessor=predecessor)
                ...
                if self._val_ratio > 0:  # create a validation set out of the test set
                    ...
                    yield Split(run=run, num=++num, train_idx=train[:n_v], test_idx=test, val_idx=train[n_v:], component=component, predecessor=predecessor)
                else:
                    yield Split(run=run, num=++num, train_idx=train, test_idx=test, val_idx=None, component=component, predecessor=predecessor)
                ...
                for i in range(len(self._index_list)):
                    ...
                    yield Split(run=run, num=++num, train_idx=train, test_idx=test, val_idx=val, component=component, predecessor=predecessor)
